...
void SetTime(void)
{
    ...
    HAL_RTC_SetTime(&RTC_Handler,&nTime,RTC_FORMAT_BCD);

    printf("Time--%d:%d:%d\r\n",nTime.Hours,nTime.Minutes,nTime.Seconds);
}
MSH_CMD_EXPORT(SetTime,SetTime);
void SetAlarm(void)
{
    //设置闹钟
    aTime.Alarm = 0;
    ...
    aTime.AlarmTime.Seconds = 0x09;
    HAL_RTC_SetAlarm_IT(&RTC_Handler,&aTime,RTC_FORMAT_BCD);
    printf("Alarm--%d:%d:%d\r\n",aTime.AlarmTime.Hours,aTime.AlarmTime.Minutes,aTime.AlarmTime.Seconds);
}
MSH_CMD_EXPORT(SetAlarm,SetAlarm);
...
void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *nhrtc) //句柄需要取新名避免与hrtc冲突
{
  ...
  rt_kprintf("Alarm is Now\r\n");
  HAL_RTC_SetAlarm_IT(nhrtc,&aTime,RTC_FORMAT_BIN);
//--------------------------------------------------------
}
